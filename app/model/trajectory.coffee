...
class Trajectory
  ...
  moveForward: (distance) ->
    ...
    if @current.lane and not @isChangingLanes and not @car.nextLane
      @car.pickNextLane()
    for id, building of @current.lane.road.buildings
      if @current.position > building.carSpawnPos - 5 && @current.position < building.carSpawnPos + 5 && @car.readyToGoHome
        console.log 'killed'
        @car.allive = false
        world.removeCar @car
