...
class World
  ...
  load: (data) ->
    ...
    for id, building of data.buildings
      @addBuilding Building.copy building
  ...
  addRandomCar: ->
    ...
    road = _.sample @roadsWithBuilgings
    if road?
      building = _.sample road.buildings
      lane = _.sample road.lanes
      @addCar new Car lane, building.carSpawnPos if lane?

  addCarToLane: (lane, position) ->
    if @carsNumber < 200
      @carsNumber += 1
      @addCar new Car lane, position
