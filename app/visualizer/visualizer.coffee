...
ToolBuildingBuilder = require './building-builder'
...
class Visualizer
  constructor: (@world) ->
    ...
    @toolBuildingBuilder = new ToolBuildingBuilder this, true
  ...
  drawBuilding: (building, alpha) ->
    color = building.color or settings.colors.building
    @graphics.drawRect building.rect
    @ctx.lineWidth = 0.8
    @graphics.stroke settings.colors.roadMarking
    @graphics.fillRect building.rect, color, alpha
  ...
  drawRoad: (road, alpha) ->
    ...
    @graphics.stroke 'blue'#settings.colors.roadMarking

    rightLine = road.rightmostLane.rightBorder
    @graphics.drawSegment rightLine
    @graphics.stroke 'red' #settings.colors.roadMarking
    @ctx.restore()
  ...
  drawGrid: ->
    ...
    for i in [box.left()..box.right()] by gridSize
      for j in [box.top()..box.bottom()] by gridSize
        ...
        @graphics.fillRect rect, settings.colors.gridPoint, null, { showCoords: true }
  ...
  draw: (time) =>
    ...
    if delta > 30
      ...
      @zoomer.transform()
      @drawGrid()
      for id, building of @world.buildings.all()
        @drawBuilding building, 0.9
